...
export const draftsHandlers = [
  // GET /api/v1/generation/drafts/:id - 获取草稿详情
  http.get('/api/v1/generation/drafts/:id', ({ params }) => {
    const { id } = params

    const mockDrafts: Record<string, CaseDraft> = {
      'draft-001': {
        id: 'draft-001',
        taskId: 'task-001',
        projectId: 'project-1',
        ...
        aiMetadata: {
          ...
          referencedChunks: [
            {
              chunkId: 'chunk-001-abc123',
              documentId: 'doc-001',
              documentTitle: '用户注册模块 PRD',
      ...
      'draft-002': {
        ...
        aiMetadata: {
          confidence: 'medium',
          modelVersion: 'claude-3.5-sonnet',
          generatedAt: '2026-04-20T10:01:00Z',
          referencedChunks: [],
        },
      },
      'draft-003': {
    ...
    const draft = mockDrafts[id as string]
    if (!draft) {
      return HttpResponse.json({ error: '草稿不存在' }, { status: 404 })
    }
